...
/* 
  Web에서 상대 경로 기준은 현재 화면이 표시되고 있는 URL (주소창의 URL)
  그래서 cafe.js 를 기준으로 경로를 설정하면 안된다.
  cafe.js는 index.html에서 사용하므로 index.html 을 기준으로 해야 한다.
*/
...
function programmingBook(data){
  let content = 
    ...
    // 전송 받은 xml을 DOM tree로 구성하기 위해 DOMParser 객체를 생성
    ...
    // book xml을 반복
    // book 태그에서 isbn 태그 찾기
    books.forEach(book => { 

    content +=  `<li>
                  <div class="menu_item">
                    <div class="menu_item_img">
                      <img src="img/book/${book.querySelector('isbn').textContent}.png" alt="${book.querySelector('title').textContent}">
                    </div>

                    <div class="menu_item_info">
                      ${book.querySelector('title').textContent} <br> (${book.querySelector('price').textContent}원)
                    </div>
                  </div>
                </li>`
    });
